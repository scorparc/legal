...
FEATURES = {
    ...
    "bonsafe": {
        "de": [
            ...
            ("Datenlöschung", "Einzelne Belege löschst du in der App. Alle lokalen Daten entfernst du durch Deinstallation oder über \"Speicher löschen\" in Android. Ein Drive-Backup löschst du in der App oder in deinem Google-Konto unter Google Drive → Einstellungen → Apps verwalten → BonSafe. Gelöschte Belege wandern zunächst in den Papierkorb und bleiben dort 30 Tage lang wiederherstellbar; danach werden sie samt ihren Fotos endgültig vom Gerät entfernt. Im Papierkorb kannst du sie auch sofort endgültig löschen."),
        ...
        "en": [
            ...
            ("Deleting data", "Delete individual receipts in the app. Remove all local data by uninstalling or using Android's Clear storage option. Delete a Drive backup in the app or in your Google account under Google Drive → Settings → Manage apps → BonSafe. Deleted receipts first move to the trash and stay restorable there for 30 days; after that they are permanently removed from the device together with their photos. You can also delete them permanently right away from the trash."),
        ...
        "es": [
            ...
            ("Eliminación de datos", "Puedes eliminar recibos individuales en la aplicación. Elimina todos los datos locales desinstalando la aplicación o usando la opción de borrar almacenamiento de Android. Elimina la copia de Drive en la aplicación o en tu cuenta de Google, en Google Drive → Ajustes → Gestionar aplicaciones → BonSafe. Los recibos eliminados van primero a la papelera y se pueden restaurar durante 30 días; después se borran definitivamente del dispositivo junto con sus fotos. También puedes eliminarlos de forma definitiva desde la papelera."),
        ...
        "fr": [
            ...
            ("Suppression des données", "Supprime les tickets individuels dans l'application. Supprime toutes les données locales en désinstallant l'application ou via l'option d'effacement du stockage Android. Supprime une sauvegarde Drive dans l'application ou dans ton compte Google, sous Google Drive → Paramètres → Gérer les applications → BonSafe. Les tickets supprimés vont d'abord à la corbeille et y restent restaurables pendant 30 jours ; ils sont ensuite définitivement effacés de l'appareil avec leurs photos. Tu peux aussi les supprimer immédiatement depuis la corbeille."),
        ...
        "it": [
            ("Backup Google Drive facoltativo (funzione Pro)", "Se utilizzi attivamente il backup, le ricevute, comprese le foto, vengono caricate come file nell'area nascosta appDataFolder del tuo account Google Drive. Il trasferimento è crittografato (TLS). Non abbiamo accesso al backup. Google Ireland Limited tratta i dati dell'account come titolare autonomo secondo la propria informativa. L'app riceve il tuo indirizzo e-mail per mostrare l'account collegato. Puoi interrompere il backup, disconnetterti ed eliminarlo. Una volta configurato, l'app aggiorna il backup automaticamente all'avvio quando l'ultimo risale a più di sette giorni fa. Viene sempre sovrascritto lo stesso file; le versioni precedenti non vengono conservate."),
            ...
            ("Cancellazione dei dati", "Elimina le ricevute singole nell'app. Rimuovi tutti i dati locali disinstallando l'app o usando l'opzione per cancellare lo spazio di archiviazione di Android. Elimina un backup Drive nell'app o nel tuo account Google, in Google Drive → Impostazioni → Gestisci app → BonSafe. Gli scontrini eliminati finiscono prima nel cestino e restano ripristinabili per 30 giorni; poi vengono rimossi definitivamente dal dispositivo insieme alle foto. Dal cestino puoi anche eliminarli subito in modo definitivo."),
        ...
        "pt": [
            ...
            ("Eliminação de dados", "Elimina recibos individuais na aplicação. Remove todos os dados locais desinstalando a aplicação ou usando a opção de limpar o armazenamento do Android. Elimina uma cópia do Drive na aplicação ou na tua conta Google, em Google Drive → Definições → Gerir aplicações → BonSafe. Os recibos eliminados vão primeiro para a reciclagem e podem ser repostos durante 30 dias; depois são removidos definitivamente do dispositivo juntamente com as fotografias. Na reciclagem também os podes eliminar de imediato."),
...
# Per-app override for the date shown to users. Only apps whose text actually
...
def make_privacy(app_slug: str, language: str) -> dict:
    ...
    return {
        ...
        "app": APPS[app_slug],
        "stand": APP_STANDS.get(app_slug, {}).get(language, STANDS[language]),
        "sections": sections,
    }
